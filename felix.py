...
DASHBOARD = {
     ...
     "AGENT_MODES": [4, 3, 2, 1], # must be of length NUM_OF_PLAYER
     # 0: manual
     # 1: random_agent
     # 2: naive_agent
     ...
     "AGENT_NAMES": ["rl", "svm", "rl", "rl"], # must be of length NUM_OF_PLAYER
     ...
     for agent_name in DASHBOARD["AGENT_NAMES"]:
          if (agent_name == ""):
               ...
               if (agent_name == "svm"):
                    ...
                    AGENT_WAREHOUSE[agent_name]["sell"] = rl_agent.get_agent()
                    AGENT_WAREHOUSE[agent_name]["bid"] = agent.SVMAgent(targetType = "bid")
               else:
